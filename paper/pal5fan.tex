...
The mock stream evolved in a potential including the  ``massive bar'' (M$_{\rm bar} = 1\times 10^{10}~\msun$, $\Omega_b = 45~\kmskpc$), demonstrates a scenario in which the leading arm has been widened by the bar (fourth row), to a similar width as observed in the data. In this scenario, the leading arm has not been truncated and there is a high number density of stars in the leading as well as trailing arm. The bar has, however, induced over- and under-densities throughout both the leading and trailing arm (third row). While this is also seen in the data, the mock-stream visualized here does not follow the ``wiggles'' of the data exactly. Interestingly, for this particular pattern speed, the bar has shifted the stream track of the trailing arm (see second row). This effect is apparent in the data on small scales where the track also appears to be ``wiggling''. Thus, this mock stream demonstrates a scenario in which both the leading and trailing arms have been perturbed by the bar, and where the leading arm width is similar to the width of the data.
...
One-dimensional modeling of the debris' spatial distribution revealed significant changes in the width and density along the tidal tails, as well as changes in their direction.
Stream simulations in different gravitational potentials separately capture many of the features observed in Pal~5, including an asymmetry in the length and the width of the leading and trailing tail.
