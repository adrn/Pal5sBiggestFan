...
Figure~\ref{fig:sims} shows the result of running this density model on the data (blue lines and confidence intervals) compared to the four different Pal 5 simulations described in Section~\ref{sec:sim} (shown as black markers).


\section{Simulations of Pal 5's evolution}
\label{sec:sim}
To explore the mechanism that produces the observed morphology of the Pal 5 stream, we run a suite of Pal~5 simulations.
In particular, we investigate whether Pal~5's wide leading arm can be explained due to chaotic regions in the potential, or whether torques from the Galactic bar are necessary to explain the width of the leading arm. We further investigate what might have caused the length asymmetry between the leading and trailing arm, as well as the gap in the trailing arm.
...
We explore barred potentials with pattern speeds of $\Omega_b$ = ($30 - 60$) $\kms$ kpc$^{-1}$ in increments of 0.5 $\kms$ kpc$^{-1}$, and we test bar masses of M$_{\rm bar} = 5 \times 10^{9}$ $\msun$ and M$_{\rm bar} = 1 \times 10^{10}$ $\msun$ (\citealt{Portail:2017}).
...
\citealt{Pearson:2017}, \citealt{Erkal:2017}, \citealt{Banik:2019}), the bar will have a too strong quadrupole for faster pattern speeds, and a too weak bar quadrupole for  slower pattern speeds. \sa{Att. APW I checked in potential.py to see if we do this correctly. Can you check also? It looks like we just set the r$_s$ in the SCF potential to the scaling in eq. \ref{eq:scale}.}
...
In Figure \ref{fig:sims} we summarize the results of our suite of Pal~5 simulations. In the top row we present the morphology of the simulated Pal~5 mock streams evolved in the static potentials with two different flattenings ($q_z = 0.94$ and $q_z = 0.5$) as well as two examples of mock-streams evolved in the ``light'' (M$_{\rm bar} = 5\times 10^9$ M$_{\odot}$) and ``massive'' (M$_{\rm bar} = 1\times 10^{10}$ M$_{\odot}$) time-dependent barred potentials. All mock-streams are visualized in the coordinate system aligned with Pal 5's tidal tails (see also Figure \ref{fig:cmds}), and the leading arm is located at positive $\phi_1$. The three bottom rows of Figure \ref{fig:sims} show the stream track (second row), stream number density (third row), and the stream width (fourth row) of the simulated mock-streams (black) as well as of the data (blue shaded region). 
...
We used the flattened ($q_z = 0.94$) static potential (left column, Figure \ref{fig:sims}) to fit the track of the stream (see Section \ref{sec:modeling}), and it is therefore not surprising that the track (second row) of the mock-stream evolved on a regular orbit (left column, black) looks similar to the track of the data (blue).  However, while the data (blue) shows variability in the number density of stars (third row), the ``Regular" mock-stream (black) has a similar number of stars in both the trailing a the leading arm. Additionally, the width of both the leading and the trailing mock-stream are too narrow for |$\phi_1| < 5$ as compared to the data. While the overall trends of width for the trailing arm at larger distances from the cluster (negative $\phi_1$) are reproduced, the leading arm of the simulated stream is too narrow as compared to the data. 

For the mock-stream evolved on a chaotic orbit (second column), 
% in the static potential with a highly flattened halo  ($q_z = 0.5$), 
the track of the leading arm (second row, black) deviates slightly from the observed stream track (blue) indicating that the flattening of the halo shape is extreme in this case. We chose the extreme flattening ( $q_z = 0.5$) in order to investigate the morphology of a stream evolved on a chaotic orbit. Interestingly the number density of stars is quite symmetric for the leading and the trailing arm in the chaotic case which was also seen for the regular orbit (third row, black). While the width (fourth row) of the leading arm is slightly wider for the chaotic mock-stream than for the mock-stream evolved on a regular orbit (left), the mock-stream width is still too narrow when compared to the leading arm data (blue). The trailing arm, on the other hand, is much wider for the chaotic mock-stream than for the data. Thus, the chaotic orbit induces a widening of both the leading and trailing arm, but this widening does not mimic the asymmetric widening of the data (blue). 

We selected the two specific examples of mock-streams evolved in barred potentials (see ``Light bar'' and ``Massive'' columns in Figure \ref{fig:sims}) through visual inspection of the morphology, track and width of the simulated streams. The ``Light bar''  (M$_{\rm bar} = 5\times 10^{9}$ M$_{\odot}$, $\Omega_b = 38 ~\kms$ kpc$^{-1}$) mock-stream simulation demonstrates a scenario in which the morphology of the mock-stream shows a leading arm which has been abruptly truncated in its density as compared to the trailing arm. This truncation is apparent in both the number density of stars (third row), as well as in the width of the mock stream (fourth row). In particular, the number of stars in the leading arm at small distances from the progentior |$\phi_1| < 2.5$ is low as compared to the trailing arm, which isn't the case fio the data (blue). We note that the width of the trailing mock-stream matches the observed data quite well, while the part of the leasding arm that has not been truncated also follows the overall trends of the data. 

The mock stream evolved in a potential including the  ``Massive bar'' (M$_{\rm bar} = 1\times 10^{10}$ M$_{\odot}$, $\Omega_b = 45 ~\kms$ kpc$^{-1}$), demonstrates a scenario in which the leading arm has been widened by the bar (fourth row), to a similar width as for the data. In this scenario, the leading arm has not been truncated and there is a high number density of stats in the leading as well as trailing arm. The bar has, however, induced over and under densities throughout both the leading and trailing arm (third row). While this is also seen in the data, the exact bar mock stream visualize there does not follow the wiggles of the data exactly. Interestingly, for this particular pattern speed, the bar has offset the stream track of the trailing arm a bit, which is a trend also seen in the data (where the stream track wiggles). Thus, this mock stream demonstrates a scenario in which both the leading and trailing arms have been perturbed by the bar and where the leading arm width mimicks that of the data. 

While none of the above mock-stream simulations  reproduce all aspects of the data simultaneously, they do provide insight into the mechanisms that could be at play in perturbing the stream. In the next Section we discuss the implications of our results. 
...
\sa{Should we use the same axis limits as in Figure \ref{fig:cmds} for the top y-row (4 to $-4$ and all x-rows $-20$ to 15 deg)? \\
Should we find a way to highlight/``blow up" the wiggles in the tracks? \\
Maybe make the sim. points 10 x lager in the top row.\\
Should I have a concluding description of what we see in the figure in the last sentence of the caption? }
}
\label{fig:sims}
\end{figure*}
