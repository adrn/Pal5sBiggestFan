...
\label{sec:intro}
%%Opening sentences on streams

%%Motivate deepest data of Pal 5
	% Emphasize Pal 5 is the only cold stream for which we know the progenitor.
	% Mention other papers: Odenkirchen, Ibata, Balbinot, Bernard (PS1)
	% Mention remaining puzzles: main sequence stars,
	% is the trailing arm thinner, does the leading arm truncate?


%%Idea: Motivate through gaps in streams
	%GD1 gaps (Boanaca, Price-Whelan)., Pal 5 gaps (Yoon), but other mechanism can 
	%produce them in prograde (Amorisco, Pearson). Emphasize why GD1 is a much cleaner case than Pal 5.
	%We need depper data, and data further south to fully understand what has created the gaps in Pal 5
	
	
%%wrapping up intro: 
%In this paper we present the deepest map of Pal 5 to date, and we extend the search of stream members further south. (Explain new data briefly: DECaLS). We find non-uniformities in the stream, in particular ....(fan, asymmetries, gaps)....  We compare our data to a suite of Pal 5 simulations in various potentials using a detailed density model, to explore various mechanisms which could lead to the formation of the new observed features presented here. 
%
%The paper is structured as follows: in Section \ref{sec:data}, we present new DECaLS Pal 5 data, in Section \ref{sec:sim} we describe the Galactic potentials and Pal 5 simulations. In Section \ref{sec:density} we describe our Gaussian mixture model we use to fit the density and width of Pal 5. We show our comparisons of the simulations and data using the density model in Section \ref{sec:results}. We discuss the implications of our results and outstanding questions about Pal 5's morphology in Section \ref{sec:discussion}, and we conclude in Section \ref{sec:conclusion}. 
