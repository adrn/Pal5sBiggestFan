...
We used the flattened ($q_z = 0.94$) static potential (left column, Figure \ref{fig:sims}) to fit the track of the stream (see Section \ref{sec:modeling}), and it is therefore not surprising that the track (second row) of the mock-stream evolved on a regular orbit (left column, black) looks similar to the track of the data (blue).  However, while the data (Figure \ref{fig:sims}), left column, third row, blue) shows variability in the number density of stars, the ``Regular" mock-stream (black) has a symmetric leading and trailing arm with a similar number of stars in both directions from the cluster (black). In the fourth row of Figure \ref{fig:sims}, it is clear that the ``Regular'' mock-stream (left) does not re-produce the width of the observed stream (blue). While the overall trends of the trailing arm (negative $\phi_1$) are reproduced, the leading arm of the simulated stream is too narrow as compared to the data. 

For the mock-stream evolved on a chaotic orbit in the static potential with a highly flattened halo  ($q_z = 0.5$), the track of the leading arm (second column, second row, black) deviates slightly from the observed stream track (blue) indicating that the shape of the halo might not be this flattened in reality. 


The track (second row) of the simulated mock-stream (black) looks similar to the track of the data (blue). 
number of stars in both the leading and trailing arm 


We selected the two examples of mock-streams evolved in barred potentials (see ``Light bar'' and ``Massive'' columns in Figure \ref{fig:sims}) of through visual inspection of the morphology, track and width of the simulated streams in the barred potentials. The ``Light bar''  (M$_{\rm bar} = 5\times 10^{9}$ M$_{\odot}$, $\Omega_b = 38 ~\kms$ kpc$^{-1}$) stream simulation demonstrates a scenario in which the morphology of the mock-stream shows a leading arm which has been abruptly truncated in its density as compared to the trailing arm.  The mock stream evolved in a potential including the  ``Massive bar'' (M$_{\rm bar} = 1\times 10^{10}$ M$_{\odot}$, $\Omega_b = 45 ~\kms$ kpc$^{-1}$), on the other hand, produced a leading arm which is widened and a trailing stream which has been perturbed by the bar as well. 
...
\end{center}
\caption{\sa{Should we use the same axis limits as in Figure \ref{fig:cmds} for the top y-row (4 to $-4$ and all x-rows $-20$ to 15 deg)? Should we find a way to highlight/``blow up" the wiggles in the tracks?}
}
\label{fig:sims}
\end{figure*}
