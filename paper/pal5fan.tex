...
Specifically, we selected stars following a 11.5\,Gyr MIST isochrone with $\rm [Fe/H] = -1.3$ \citep{Choi:2016} between $20<g<23.7$.
...
Despite these challenges, the Pal~5 main-sequence is evident in field~9 ($9^\circ<\phi_1<12^\circ$), even though the leading tail truncates at $\phi_1\sim7^\circ$.
This indicates that Pal~5 tails may extend beyond the current extent, but at a much lower surface-brightness.
...
With the improved star--galaxy separation and a selection that accounts for the distance gradient along the stream, Pal~5 tails are prominent between R.A.$\sim223^\circ$ and $\sim245^\circ$ (Figure~\ref{fig:maps}).
...
% Figure~\ref{fig:maps} shows the most likely members of Pal~5
% - to refine our selection, we select with variable distance and color-color selection for stars vs galaxies
% - we measure the distance gradient along the stream from the location of pal5's ms turnoff, overplotted with the isochrone at the adopted distance
...
% - and employ the $z$-band to distinguish between faint stars and galaxies more efficiently.
