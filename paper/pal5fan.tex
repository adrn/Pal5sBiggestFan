...
\label{sec:intro}
We now know of $\sim$xx thin stellar streams in the Milky Way (MW), most of which likely are relics of disrupted globular clusters (see e.g. \citealt{Ibata:2019}). The only MW globular cluster stream, for which we know the progenitor, however, is the Palomar 5 (Pal 5) stream. We assume the other thin, stellar streams originated from globular clusters as well, due to their narrow widths, and therefore, small tidal radii. But how thin are these streams actually? To date, the deepest maps of thin stellar streams probe down to limiting magnitudes of $g \sim 22$. It is not unreasonable to expect that wider, and potentially non-uniform features will be discovered surrounding the thin stellar streams, once we look deeper. 

\citealt{Pearson:2015} predicted that in certain potentials streams could ``fan out" due to certain orbit structures within our Galaxy. Subsequently, \citet{Price-Whelan:2016} showed that this was due to mild chaotic behavior of the stream stars. Additionally, \citealt{Bonaca:2014} showed that streams should furthermore look ``puffed up" if they are evolved in a live dark matter halo with subhalos interacting with them (see also \citealt{Ibata:2002}). So maybe streams should not be as thin as they appear to look. 

To date, very few streams have been mapped to deeper depths than ...,. Hence, we are not sure that they actually are as thin and uniform as they might appear from current data. \citet{Sesar:2016} found evidence that there might be a ``fan" from the Ophiuchus stellar stream, which could arise as a consequence of the time dependent Galactic bar inducing chaotic orbits (\citealt{Price-Whelan:2016b}). The bar has also been suggested to explain the shortness of some streams (\citealt{Hattori:2016}, \citealt{Pearson:2017}) and has been theorized to produce under densities and gaps in streams (\citealt{Erkal:2017}, \citealt{Pearson:2017}), which could also lead to non-uniformities along the stream and stars off-set from the regular stream track.
