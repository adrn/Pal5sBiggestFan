...
With the improved star--galaxy separation and a selection that accounts for the distance gradient along the stream, Pal~5 tails are prominent between $\rm R.A.\sim223^\circ$ and $\sim245^\circ$ (Figure~\ref{fig:maps}).
...
The background stellar density in this region of the sky does not vary substantially over the field (see, e.g., Figure~\ref{fig:maps}), but displays some gradients with sky position that cannot be represented by a purely uniform background.
...
We presented deep $grz$ photometry of the Palomar~5 tidal tails from the Legacy Surveys catalog that enabled the cleanest selection of Pal~5 members to date.
...
Through a qualitative comparison with model stellar streams, we demonstrated that the perturbation by the Milky Way bar can produce many of the features observed in Pal~5, including the shorter length and larger width of the leading tail compared to the trailing tail.
In general, however, stream models without small-scale perturbers fail to reproduce the observed gaps in the stream density and the wiggles of its track.
In this section we discuss the perturbing mechanisms in the inner galaxy that could jointly explain the complete Pal~5 stream.
\ab{rephrase as needed once the rest of the analysis is in place}

- narrow-wide dichotomy in trailing-leading -- seems to be confirming the bar perturbation picture
- cmd implies debris spread out to even lower surface-brightness present
- quantitative analysis needed to further constrain bar properties

- density variations: confirming the large scale ones from cfht data
- erkal: large from subhalos, small from gmcs
- detect them confidently above the background -- crucial for constraining the mass and time of impact -> can test this hypothesis
% - cumulative number density?
- small scale

- wiggles: stream track $\sim$ mass enclosed
- erkal commented change in curvature of the leading tail, also not reproduced in their models
- these large scale shifts may indicate a change in orbit? sgr infall and increase in the enclosed mass?
- also see small-scale wiggles all along the trailing tail -- another signature of impacts, bar perturbation?

- ultimately, these discoveries bode well for future studies of stellar streams with photometry alone
- the community has been leveraging the gaia kinematic data to unveil a range of dynamical processes are operating in the milky way (cits)
- photometric surveys have a larger reach:
- push this kind of analysis to the outer halo (streams being discovered, cite pandas -- valuable streams for dm tests, less likely to have been affected by baryonic structures, radial info as well)
- potentially also external galaxies (cite erin for dwarf galaxy streams, sarah for discovery potential of cold streams)
