...
\label{sec:intro}
Several thin stellar streams have been discovered in our Galaxy. Supposedly, these are the relics of disrupted globular clusters based on their small tidal radii (i.e. widths) and for some streams based on their metallicities. However how thin are these streams actually? Once we look deeper many of these streams might have low-density stars surrounding them. 
...
To date, very few streams have been mapped to deeper depths than ...,. Hence, we are not sure that they actually are as thin and uniform as we might expect from current data. \citet{Sesar:2016} found evidence that there might be a ``fan" from the Ophiuchus stellar stream, which could arise as a consequence of the time dependent Galactic bar inducing chaotic orbits (\citealt{Price-Whelan:2016b}). Additionally, torques to streams (\citealt{Hattori:2016}, \citealt{Erkal:2017}, \citealt{Pearson:2017}) which could also lead to non-uniformities along the stream and stars off-set from the regular stream track.
