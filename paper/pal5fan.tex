...
In the second row, we compare the inferred stream track (solid blue line and confidence interval) to the second-order polynomial fits of the leading and the trailing arm (dotted line).
While these low-order polynomials capture the general curvature of Pal~5, the stream track sharply changes direction at several locations.
These wiggles are more easily evident in track deviation from the polynomial fits (third row).
Two of the wiggles appear almost symmetrically in the leading and trailing tails at $\phi_1\approx3.5^\circ$ and $\phi_1\approx-4.5^\circ$, respectively, while the trailing tail has another wiggle at $\phi_1\approx-13^\circ$.

% In rows 2 to 5, blue lines and confidence intervals show the inferred stream track, its residuals from second-order polynomial fits to the leading and trailing arm, the total number of member stars, and width, respectively.

To summarize, 
In Section~\ref{sec:sim} we run the same density model on four different Pal~5 simulations and compare it to the observed Pal~5 results in Figure~\ref{fig:sims}.
