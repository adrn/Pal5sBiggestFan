...
data {
    ...
    // nodes locations along rigid polynomial
		vector[n_nodes] bg_phi1_nodes;
		vector[n_nodes] bg_phi2_nodes;
}
...
parameters {
    ...
    vector<lower=1, upper=8.>[n_nodes] bg_log_w_nodes;
    vector<lower=1, upper=8.>[n_nodes] bg_log_h_nodes;
    vector<lower=-8, upper=8>[n_nodes] bg_log_a_nodes;
}
...
transformed parameters {
    ...
    vector[n_bg_nodes] bg_w_nodes;
		vector[n_bg_nodes] bg_h_nodes;

    matrix[2,2] C[n_nodes];
    matrix[2,2] C_bg[n_nodes];

    // Background
    ln_bg_val = log(bg_val);
    bg_w_nodes = exp(bg_log_w_nodes);
    bg_h_nodes = exp(bg_log_h_nodes);
	  ...
	  for (i in 1:n_pix) {
        ...
        for (j in 1:n_bg_nodes) {
            node_xy[1] = bg_phi1_nodes[j];
						node_xy[2] = bg_phi2_nodes[j];

            C_bg[j] = get_cov(bg_h_nodes[j], bg_w_nodes[j]);
						tmp_bg[j] = bg_log_a_nodes[j] + multi_normal_lpdf(xy | node_xy, C_bg[j]);
        }
...
model {
    ...
    // Priors
    for (n in 1:n_bg_nodes) {
        bg_log_w_nodes[n] ~ normal(-1, 0.2)T[1, 8.];
        bg_log_h_nodes[n] ~ normal(-1, 0.2)T[1, 8.];
    }
